	...
	<v-card outlined id="v-filter-form" class="sticky-top">
		...
		<form method="GET" :action="`${url}/vehicle/filter`">
			<v-card
				...
				color="white"
				flat
				class="row pt-4 pr-0 pb-0 pl-4 m-0 d-flex justify-content-center"
			>
				<div class="col-12 pt-0 pr-4 pb-0 pl-0">
					<v-select name="make" outlined dense label="Make" v-model="make" :items="convertedMakeModels"></v-select>
				</div>
				<div class="col-12 pt-0 pr-4 pb-0 pl-0">
					<v-select name="model" outlined dense label="Model" :items="models" v-model="request.model"></v-select>
				</div>
				<div class="col-12 pt-0 pr-4 pb-0 pl-0">
					<v-select
						name="min_year"
				...
				<div class="col-12 pt-0 pr-4 pb-0 pl-0">
					<v-select name="fuel" outlined dense label="Fuel" :items="options.fuel" v-model="request.fuel"></v-select>
				</div>
				<div class="col-12 pt-0 pr-4 pb-0 pl-0">
					<v-select
						name="condition"
				...
				<div class="col-12 pt-0 pr-4 pb-0 pl-0">
					<v-text-field name="max_price" outlined dense label="To Price (GYD)" v-model="request.max_price"></v-text-field>
				</div>
				<div class="col-12 pt-0 pr-4 pb-4 pl-0 d-flex justify-content-center">
		...
		<form v-if="false" id="logo-filter" class="row mx-0 mb-0 mt-4" method="GET" :action="`${url}/vehicle/filter`">
			...
			<div class="col-2">
				<v-img @click="filterByLogo('make','Nissan')" aspect-ratio="1.5" :src="`${url}/images/nissan_logo.png`" contain style="cursor: pointer;"></v-img>
			</div>
			<div class="col-2">
				<v-img @click="filterByLogo('make','Honda')" aspect-ratio="1.5" :src="`${url}/images/honda_logo.jpg`" contain style="cursor: pointer;"></v-img>
			</div>
			<div class="col-2">
				<v-img @click="filterByLogo('make','Bmw')" aspect-ratio="1.5" :src="`${url}/images/bmw_logo.png`" contain style="cursor: pointer;"></v-img>
			</div>
			<div class="col-2">
				<v-img @click="filterByLogo('make','Mercedes')" aspect-ratio="1.5" :src="`${url}/images/mercedes_benz_logo.jpg`" contain style="cursor: pointer;"></v-img>
			</div>
			<div class="col-2">
...
export default {
	...
	methods: {
		filterByLogo(name, value) {
			...
			let filterByLogoInput = document.getElementById("filter-by-logo-input");
