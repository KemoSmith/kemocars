...
export default {
            ...
            axios({
                ...
                .then(response => {
                    ...
                    window.location.replace(
                        `/vehicle/${response.data.vehicle.id}`
                    );
                })
                .catch(errors => {
                    console.log(errors.response);
    ...
    computed: {
        ...mapGetters(["showPostVehicle", "isDarkMode", "options"]),
        convertedMakeModels() {
            let converted = [];
        ...
        imagesAmount() {
            let selecedAmount = this.newVehicleImages.length;
